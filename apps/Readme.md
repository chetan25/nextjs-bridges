...
stitched into the shell at runtime via `@bridge/share` — a small,
...
(`@bridge/share/next-config-helper`), which writes `public/share-manifest.json` —
...
**`@bridge/lazy-handler` — bundled into every consumer, not shared.**
...
**`@bridge/hydration` — shell-only, never shipped to a remote.**
